...
-- |An implementation of unsigned 10-bit words. These are used to identify
-- event channels.
module Data.Word10(Word10) where
...
newtype Word10 = Word10 {unWord10 :: Word32}
...
instance Bounded Word10 where
  minBound = 0
  maxBound = wrap maxVal

instance Real Word10 where
...
{-# RULES
"fromIntegral/Word10->Word32" fromIntegral = fromIntegral . unwrap
"fromIntegral/Word10->Int"    fromIntegral = fromIntegral . unwrap
"fromIntegral/Word10->Word10" fromIntegral = id :: Word10 -> Word10
  #-}

instance Ix Word10 where
  range (m,n)                    = [m..n]
  index b@(m,_) i  | inRange b i = fromIntegral (i - m)
                   | otherwise   = error "Error in array index"
  inRange (m,n) i                = m <= i && i <= n

instance Read Word10 where
  readsPrec p s = [(fromInteger x, r) | (x, r) <- readsPrec p s] 

instance Integral Word10 where
